...
| Whole-vault sweep (Q13) | `uv run python -m learn_to_ship recall --all` — run it keyless (format-only); with a key it's one LLM call per card | every card file reviewed, grouped per file | ☑ 2026-07-09 (54 files / 111 cards) |
